    ...
    public class GunGame : Script
    {
        ...
        static void RE_disconnectlobby(Player player)
        {
            try
            {
                ...
                NeptuneEvo.Trigger.ClientEvent(player, "client::removeweapon", GunGame.WeaponOnName[arena.Weapon]);
                ...
                if (arena.Owner == player)
                {
                    ...
                    foreach (Player ply in arena.Players)
                    {
                        ...
                        NeptuneEvo.Trigger.ClientEvent(ply, "client::removeweapon", GunGame.WeaponOnName[arena.Weapon]);
        ...
        static void RE_sendlobby(Player player, string LobbyInfoT)
        {
            try
            {
                ...
                NeptuneEvo.MoneySystem.Wallet.Change(player, -Convert.ToInt32(LobbyInfo[2]));
        ...
        public void Stop()
        {
            try
            {
                ...
                foreach (KeyValuePair<Player, int> pair in ForSort.Reverse().ToDictionary(x => x.Key, x => x.Value))
                {
                    ...
                    if (i == 1)
                        NeptuneEvo.MoneySystem.Wallet.Change(pair.Key, Money);
                    Winners.Add(Winner);
                }
